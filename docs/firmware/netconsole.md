...
- `ipaddr` must be non-zero or `net_loop()` refuses NETCONS outright
  ...
  192.168.1.10 is a fallback for a single camera on 192.168.1.0/24, not a
  fleet policy. Like the other two lines it is written only when nothing else
  set one, so all of these take precedence: `ipaddr=` in the camera's
  `uenv.txt`, in `user/<camera>/local.uenv.txt`, or `fw_setenv ipaddr` on an
  ...
  too, so the consoles interleave on one listener. Give each camera a unique
  `ipaddr`, and pin `ncip` to your host -- `ncip` disambiguates input, but the
  ARP reply still needs the address to be unique.
